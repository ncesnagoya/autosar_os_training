	...
	for(;;) {
		...
		if(dsw_state != pre_dsw_state){
			...
			if(((dsw_state & DSW1) == DSW1) && ((pre_dsw_state & DSW1) != DSW1)){
				syslog(LOG_INFO, "main_task : GetResource(RES_TASK).");
				GetResource(RES_TASK);
			}

			if(((dsw_state & DSW1) != DSW1) && ((pre_dsw_state & DSW1) == DSW1)){
				syslog(LOG_INFO, "main_task : ReleaseResource(RES_TASK).");
				ReleaseResource(RES_TASK);
			}

			if(((dsw_state & DSW2) == DSW2) && ((pre_dsw_state & DSW2) != DSW2)){
				syslog(LOG_INFO, "main_task : GetResource(RES_INT_PSW1).");
				GetResource(RES_INT_PSW1);
			}

			if(((dsw_state & DSW2) != DSW2) && ((pre_dsw_state & DSW2) == DSW2)){
				syslog(LOG_INFO, "main_task : ReleaseResource(RES_INT_PSW1).");
				ReleaseResource(RES_INT_PSW1);
			}

			if(((dsw_state & DSW3) == DSW3) && ((pre_dsw_state & DSW3) != DSW3)){
				syslog(LOG_INFO, "main_task : GetResource(RES_INT_PSW2).");
				GetResource(RES_INT_PSW2);
			}

			if(((dsw_state & DSW3) != DSW3) && ((pre_dsw_state & DSW3) == DSW3)){
				syslog(LOG_INFO, "main_task : ReleaseResource(RES_INT_PSW2).");
				ReleaseResource(RES_INT_PSW2);
			}
