...
TASK(main_task){
	...
	for(;;) {
		WaitEvent(EVENT_PSW1|EVENT_PSW2);
		GetEvent(main_task, &mask);
		ClearEvent(mask);

		if((mask & EVENT_PSW1) == EVENT_PSW1){
			syslog(LOG_INFO, "main_task : PSW1 Pushed.");
		}

		if((mask & EVENT_PSW2) == EVENT_PSW2){
			syslog(LOG_INFO, "main_task : PSW2 Pushed.");
		}
	}
}	/* TASK( main_task ) */
