...
TASK(dsw_task){
	...
	for(;;) {
		...
		if(dsw_state != pre_dsw_state){
			...
			/* ログ出力 */
			syslog(LOG_INFO, "DipSwitch State = 0x0%x", dsw_state);
			syslog(LOG_INFO, "LED State       = 0x0%x", led_state);
		}
		...
		/* 状態変化判定 */
		if(psw_state != pre_psw_state){
			syslog(LOG_INFO, "PSW State Change = 0x0%x to 0x0%x", pre_psw_state, psw_state);
